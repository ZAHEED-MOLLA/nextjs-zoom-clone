...
function SetupUI({ onSetupComplete }: SetupUIProps) {
    ...
    const [micCamDisabled, setMicCamDesabled] = useState(false);
    ...
    return (
        <div className="flex flex-col items-center gap">
            ...
            <div className="flex h-16 items-center gap-3">
                <AudioVolumeIndicator /> {/* Include AudioVolumeIndicator here */}
                <DeviceSettings />
            </div>
            <label className="fle items-center gap- font-medium">
                <input
                    type="checkbox"
                    checked={micCamDisabled}
                    onChange={(e) => setMicCamDesabled(e.target.checked)}
                />
